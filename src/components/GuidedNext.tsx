  ...
  return (
    <section className="mt-10 border-t border-(--border) pt-6">
      <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          <div className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.22em] text-(--accent)">Suggested Next Paths</div>
          <h4 className="font-syne text-lg font-black leading-tight text-(--text)">{currentGoal.coreQuestion}</h4>
        </div>
        <p className="max-w-md text-xs leading-relaxed text-(--text-muted)">{currentGoal.intentDescription}</p>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-(--accent)">{formatViewLabel(view)}</span>
